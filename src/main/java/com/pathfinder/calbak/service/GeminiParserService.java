...
public class GeminiParserService {
    ...
    public List<ParsedResponse> parseSchedule(String rawText, List<MultipartFile> images) {
        ...
        String prompt = """
            ...
            - 각 수업의 `startDate`는 위에서 구한 학기 시작일 이후 '가장 먼저 도래하는 해당 수업의 요일' 날짜로 설정해. 단, 파싱 시점(오늘 날짜)이 학기 시작일보다 이미 지났다면, 과거가 아닌 '오늘 날짜 이후 가장 먼저 도래하는 해당 수업 요일'을 `startDate`로 설정해.
            - 각 수업의 `repeatEndDate`는 총 16회차 반복을 정확히 맞추기 위해, 학기 시작일 기준 첫 수업일로부터 15주(105일) 뒤의 날짜로 설정해 (112일 아님).
